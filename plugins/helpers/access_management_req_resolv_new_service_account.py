...
def fetch_activity_info_new_service_account(**context) -> Dict[str, Any]:
    """
    Fetch activity information for new service account requests from EXT_ACCESS_MANAGEMENT_ACTIVITY table.

    This function:
    1. Gets activity_data from update_activity_data_wo_approved task via XCom
    2. Queries EXT_ACCESS_MANAGEMENT_ACTIVITY table for request details (fields: u_wo_no, u_svc_name, u_user_name, u_user_email, u_role, u_env, u_task_type)
    ...
    # Query to fetch required fields for new service account (updated fields)
    query = f"""
        SELECT
            u_wo_no,
            u_svc_name,
...
def ad_process_new_service_account(**context) -> Dict[str, Any]:
    """
    Process AD operations for new service account request.

    This function:
    1. Gets activity_data from fetch_activity_info_new_service_account task via XCom (which includes activity_info_fetched with fields wo_no, svc_name, user_name, user_email, role, env, task_type).
    ...
    Input (from fetch_activity_info_new_service_account task's activity_data):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
            ...
            # Processed AD output
            'ad_output_new_service_account': 'Service account created successfully<br>Password reset completed<br>Account enabled',  # HTML formatted
    ...
    Note: All processed fields (ad_service_account_list, ad_new_password, ad_output_new_service_account) 
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting AD new service account process")
    ...
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.fetch_activity_info_new_service_account', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")
    ...
    hook.info(f"Processing new service account creation for service: {activity_info['svc_name']}, work order: {activity_info['wo_no']}")
    ...
    hook.info("Creating input file for new service account")
    # Create input file command (updated as per legacy command)
    input_command = f'''echo {{"inputvalues":[{{"WO_NO":"{activity_info['wo_no']}","SVC_FULL_NAME":"{activity_info['svc_name']}","SVC_FIRST_NAME":"{activity_info['svc_name']}","SVC_ADID":"{activity_info['svc_name']}","SVC_OWNER_EMAIL":"{activity_info['user_email']}","SVC_OWNER_NAME":"{activity_info['user_name']}","SVC_ROLE":"{activity_info['role']}"}}],"action_name":"New Service Account Creation"}} > "C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\NewSvcAcc\\newsvcacc_i_{process_id}.json"'''

    script_cmd = '"C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\new_svc_creation.bat"'
    output_file = f'C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\NewSvcAcc\\newsvcacc_o_{process_id}.json'
    ...
    with AD_ProcessHook(
        ...
        logger.info(f"AD process result data: {result['data']}")

    if result['success']:
        hook.info("New service account process completed successfully")
        ...
        if ad_output:
            ...
            activity_data['data']['ad_output_new_service_account'] = str(ad_output).replace('\\r\\n', '<br>')
        else:
            activity_data['data']['ad_output_new_service_account'] = None
        ...
        hook.error(f"New service account process failed: {result.get('error', 'Unknown error')}")
        activity_data['data']['ad_process_result'] = result['data']
...
def rs_update_activity_new_service_account(**context) -> Dict[str, Any]:
    """
    Update activity table with new service account details after successful service account creation.

    Input (from ad_process_new_service_account task's activity_data):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
            ...
            'ad_service_account_list_raw': 'svc1',
            'ad_service_account_list': '"svc1"',
            'ad_new_password_raw': 'P@ssw0rd123',
            'ad_new_password': 'P@ssw0rd123',
            'ad_email_password': 'P@ssw0rd123',
            'ad_output_new_service_account': 'Service account created successfully<br>Account enabled<br>Password set',
            'ad_python_status': 'Finished',
            'ad_python_exception': None,
            'activity_update_result_new_service_account': {
                'status': 'Updated',
                'message': 'Activity updated with new service account details',
                'timestamp': '2024-06-07T12:34:56.789'
            }
        }
    }

    Output (complete data structure with activity_update_result_new_service_account added):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    svc_name = activity_data['data']['activity_info_fetched']['svc_name']

    hook.info(f"Updating activity for process_id: {process_id} with new service account details")
    ...
    query = f"""
        ...
        SET u_new_password = '{new_password}',
            u_status = 'Completed',
            u_remarks = 'Service account created successfully.'
    ...
    if not result.get('success'):
        ...
        activity_data['data']['activity_update_result_new_service_account'] = {
            'status': 'Failed',
            'error': error_msg,
...
def send_notification_new_service_account_create(**context) -> Dict[str, Any]:
    """
    Send email notifications for new service account creation.

    This function:
    1. Gets activity_data from rs_update_activity_new_service_account task via XCom
    ...
    Input (from rs_update_activity_new_service_account task's activity_data):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    Output (extends activity_data with notification_result):
    {
        'data': {
            ...
            'notification_result_new_service_account_create': {
                'status': 'Sent',
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for new service account creation")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.rs_update_activity_new_service_account', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")
    ...
    if not user_email:
        ...
        activity_data['data']['notification_result_new_service_account_create'] = {
            'status': 'Not Sent',
            'recipients': [],
            'timestamp': datetime.now().isoformat(),
    ...
    hook.info(f"Sending new service account notification to {user_email}")
    ...
    try:
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            ...
            mapping_key="new_service_account_notification_email_template"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for new service account notification template")
        ...
        subject = f"Access Management | Service Account Creation | Process Id: {identifier}"
...
def check_shift_rooster_new_service_account_rs(**context) -> Dict[str, Any]:
    """
    Check shift roster for new service account request and prepare shift engineer notification.

    This function:
    1. Gets activity_data from send_notification_new_service_account_create task via XCom
    ...
    Input (from send_notification_new_service_account_create task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    Output (extends activity_data with shift_roster_result_new_service_acc):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
            ...
            'shift_roster_result_new_service_acc': {
                'status': 'Found',
                'shift_engineers': [
                    {
                        'name': 'John Smith',
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting shift roster check for new service account request")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.send_notification_new_service_account_create', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")

    # Get current time in UTC (server time)
    current_time = datetime.now(timezone.utc)
    # Convert to PST for shift calculations (UTC + 5:30)
    ...
    try:
        ...
        if not result:
            activity_data['data']['shift_roster_result_new_service_acc'] = {
                'status': 'Not Found',
                'shift_engineers': [],
                'email_list': '',
...
def send_communication_shift_wise_fo_new_service_account(**context) -> Dict[str, Any]:
    """
    Send email notification to shift engineers about new service account creation.

    This function:
    1. Gets worklog_id from XCom
    2. Gets service account data and shift roster details from activity_data
    3. Prepares email content using template from mapping
    4. Sends email notification to shift engineers using email hook

    Input (from check_shift_rooster_new_service_account_rs task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    Output (extends activity_data with shift_communication_result_new_service_acc):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
            ...
            'shift_communication_result_new_service_acc': {
                'status': 'Sent',
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification to shift engineers for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.check_shift_rooster_new_service_account_rs', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")
    ...
    ad_password = activity_data['data']['ad_new_password']
    identifier = activity_data['data']['request_resolve_initial_data']['identifier']
    task_type = activity_data['data']['activity_info_fetched']['task_type']

    # Get shift engineers' email list
    shift_result = activity_data['data'].get('shift_roster_result_new_service_acc', {})
    if shift_result.get('status') != 'Found':
        hook.warning("No shift engineers found to send notification")
        activity_data['data']['shift_communication_result_new_service_acc'] = {
            'status': 'Not Sent',
            'recipients': [],
            'timestamp': datetime.now().isoformat(),
    ...
    hook.info(f"Sending new service account notification to {len(recipients)} shift engineers")
    ...
    try:
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_request_resolve",
            mapping_namespace_name="access_management_enable",
            mapping_key="shift_engineer_notification_email_template_new_service_account"
        )
...
def update_wo_comment_new_service_account(**context) -> Dict[str, Any]:
    """
    Update work order with work info comment after new service account creation.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'wo_comment_update_new_service_account': {
                'status': 'Created',
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting work log creation for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.send_communication_shift_wise_fo_new_service_account', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
    ...
    svc_name = activity_data['data']['activity_info_fetched']['svc_name']
    work_log_comment = f"Service account {svc_name} has been created in {env} environment."
    ...
    try:
        with httpx.Client(verify=False, timeout=480.0) as client:
            ...
            activity_data['data']['wo_comment_update_new_service_account'] = {
                'status': 'Created',
...
def update_wo_status_new_service_account(**context) -> Dict[str, Any]:
    """
    Update work order status after new service account creation.

    This function:
    1. Gets WO number from activity_data
    2. Searches for the work order using the API
    3. Gets the request_id from the response
    4. Updates the work order status and details after service account creation

    Input (from update_wo_comment_new_service_account task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    Output (added to activity_data):
    {
        'data': {
            // ... same as input ...
            'wo_result_update_new_service_account': {
                'status': 'Updated',
                'request_id': 'WO66813',
                'wo_number': 'WO123456'
            }
    ...
    try:
        with httpx.Client(verify=False, timeout=480.0) as client:
            ...
            logger.info(f"Work order request id: {work_order}")
            task_description = work_order.get('description')
            ...
            ref_adid = activity_data['data']['activity_info_fetched'].get('ref_adid', '')
            ...
            if task_description and "access management-uprising create account".lower() in task_description.lower():
                update_payload = {
                    "status": "Completed",
            ...
            activity_data['data']['wo_result_update_new_service_account'] = {
                'status': 'Updated',
...
def update_activity_info_wo_comment_new_service_account_err(**context) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(f"Error in update_activity_info_wo_comment_new_service_account_err: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
def send_comm_err_wo_comment_new_service_account_err(**context) -> Dict[str, Any]:
    """
    ...
    Input (from update_activity_info_wo_status_new_service_account_err task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.update_activity_info_wo_comment_new_service_account_err', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")
    ...
    try:
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_request_resolve",
            mapping_namespace_name="access_management_enable",
            mapping_key="wo_status_error_notification_email_template_service_account"
        )
        ...
        # Send email using notify hook
        subject = f"Access Management | {task_type} | Error in Work Order Status Update | Process Id: {identifier}"
        notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
        email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")
        user_email = activity_data['data']['activity_info_fetched']['user_email']
...
def update_activity_info_wo_status_new_service_account_err(**context) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.error(f"Error in update_activity_info_wo_status_new_service_account_err: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
def send_comm_err_wo_status_new_service_account_err(**context) -> Dict[str, Any]:
    """
    Send error email notification when work order status update task fails.
    ...
    # Get activity data from the error task
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.update_activity_info_wo_status_new_service_account_err', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        raise ValueError("No activity data found from previous task")
...
def send_communication_non_finished_ad_new_service_account(**context) -> Dict[str, Any]:
    """
    Send email notification for non-finished AD password reset status.

    This function:
    1. Gets worklog_id from XCom
    2. Gets activity data from ad_process_new_service_account task
    3. Prepares email content using template from mapping
    4. Sends email notification using notify hook

    Input (from ad_process_new_service_account task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
    ...
    Output (same as input with communication_result added):
    {
        'data': {
           'process_id': 'AM-20240607123456-1a2b3c4d',
            ...
            'ad_python_status': 'Not Finished',
            'ad_python_exception': 'Connection timeout',
            'communication_result_non_finished_ad_new_service_account': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'timestamp': '2024-06-07T12:34:56.789',
    ...
    try:
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_request_resolve",
            mapping_namespace_name="access_management_enable",
            mapping_key="new_service_account_non_finished_notification_email_template"
        )

        if not mapping_elements:
            hook.error("No mapping element found for new service account non-finished notification template")
            raise ValueError("No mapping element found for new service account non-finished notification template")
        ...
        email_subject = f"Access Management | Service Account Creation | Process Id:{process_id}"
        identifier = activity_data['data']['request_resolve_initial_data']['identifier']
        ...
        try:
            ...
            # Update activity_data with success information including the result
            activity_data['data']['communication_result_non_finished_ad_new_service_account'] = {
                'status': 'Sent',
                'timestamp': datetime.now().isoformat(),
...
def update_activity_info_non_finished_ad_new_service_account(**context) -> Dict[str, Any]:
    """
    ...
    # Get activity data from previous task send_communication_non_finished_ad_new_service_account
    activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.send_communication_non_finished_ad_new_service_account', key='activity_data_request_resolve')
    if not activity_data or 'data' not in activity_data:
        hook.error("No activity data found from previous task")
        return {
            "success": False,
...
def update_activity_new_service_account_decryption_failed(**context) -> Dict[str, Any]:
    """
...
def send_communication_new_service_account_decryption_failed(**context) -> Dict[str, Any]:
    """
    Send email notification when password decryption fails during new service account creation.

    This function:
    1. Gets activity data from update_activity_new_service_account_decryption_failed task
    ...
    Input (from update_activity_new_service_account_decryption_failed task's activity_data_request_resolve):
    {
        'data': {
            'process_id': 'AM-20240607123456-1a2b3c4d',
            'worklog_id': 'WL-123456',
            'request_resolve_initial_data': {
                ...
                'identifier': 'AM-20240607123456-1a2b3c4d',
                'remarks': 'Initial request',
                'task_type': 'New Service Account Creation',
                'upn': 'user@uprising.t-mobile.com',
                'flag_update_date': 0,
                'creation_date': '2024-06-07 12:34:56',
                'update_date': '2024-06-07 12:34:56',
            ...
            'ad_process_result': {
                'PYTHON_STATUS': 'Not Finished',
                'AD_OUTPUT': 'Error processing service account creation',
                'New_Password': None,
                'SVC_ACCOUNT': None,
                'PYTHON_EXCEPTION': 'Decryption failed: Invalid key format'
            },
    ...
    try:
        ti = context['ti']
        activity_data = ti.xcom_pull(task_ids='new_service_account_task_group.update_activity_new_service_account_decryption_failed', key='activity_data_request_resolve')
        if not activity_data or 'data' not in activity_data:
            raise ValueError("No activity data found from previous task")
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_request_resolve",
            mapping_namespace_name="access_management_enable",
            mapping_key="decryption_failed_notification_email_template_service_account_creation"
        )
        ...
        template_data = {
            ...
            "svc_name": activity_data['data']['activity_info_fetched']['svc_name'],
            "upn": activity_data['data']['request_resolve_initial_data']['upn'],
            "environment": activity_data['data']['activity_info_fetched']['env']
        }
