...
def fetch_data_activity_info_new_user_account(**context) -> Dict[str, Any]:
    """
    Fetch activity information for new user account creation from EXT_ACCESS_MANAGEMENT_ACTIVITY table.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'activity_info_fetched': {
                ...
                'env': 'Production',
                'description': 'New user account creation request',
                'organization': 'IT',
                'platform': 'Windows',
    ...
    """
    logger.debug(f"Fetching data activity info for new user account")
    ti = context['ti']
    ...
    logger.debug(f"Fetched activity info for process_id: {activity_identifier}| activity info {json.dumps(activity_data)}")
...
def ad_process_new_user_account(**context) -> Dict[str, Any]:
    """
    Connect to AD and execute new user account creation process.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'ad_process_result': {
                ...
                'ad_output_new_user_account': 'User Account Created Successfully'
            }
        }
    }
    """
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.fetch_data_activity_info_new_user_account', key='activity_data')
    ...
    input_command = f'echo {{"inputvalues": [{{"USER_FULL_NAME" : "{user_name}"}}]}} > "C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New User Account Creation\\checkUserAcc\\checkuseracc_i_{process_id}.json"'

    # echo {"inputvalues": [{"USER_FULL_NAME" : "$FLOW{USER_NAME}"}]}  >  "C:\Users\m2m_enable_auto\Documents\Access Management Automation\New User Account Creation\CheckUserAcc\checkuseracc_i_$PARAM{IDENTIFIER}.json"

    # Script command to execute the batch file
    script_cmd = '"C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New User Account Creation\\check_user_account.bat"'

    # "C:\Users\m2m_enable_auto\Documents\Access Management Automation\New User Account Creation\check_user_account.bat"

    # Output file paths
    output_file = f'C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New User Account Creation\\CheckUserAcc\\checkuseracc_o_{process_id}.json'

    # path:  C:\Users\m2m_enable_auto\Documents\Access Management Automation\New User Account Creation\CheckUserAcc  file : checkuseracc_o_$PARAM{IDENTIFIER}.json
    ...
    if result['success']:
        ...
        activity_data['data']['ad_output_new_user_account'] = result['data']['AD_OUTPUT']
        ti.xcom_push(key='activity_data', value=activity_data)
...
# AD process success flow when new user account being created
def sr_creation_new_user_account(**context) -> Dict[str, Any]:
    """
    Create service request for new user account.

    This function:
    1. Gets worklog_id from XCom
    2. Gets user data from activity_data
    3. Creates service request for new user account
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting SR creation for new user account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    hook.info(f"Creating SR for {task_type} for user: {user_name}")
    ...
    u_user_email = activity_row[1]
    u_manager_name = activity_row[2]
    u_manager_adid = activity_row[3]
    u_manager_email = activity_row[4]
    u_user_name = activity_row[5]
    u_organization = activity_row[6]
    u_user_adid = activity_row[7]
    u_env = activity_row[8]
    u_platform = activity_row[9]
    u_description = activity_row[10]
    u_mobile_no = activity_row[11]
    ...
    """
    logger.debug(f"Query: {approver_query}")
    ...
    sr_name = f"New User Account Creation - {u_user_name} ({u_user_adid}) - {u_organization}"
    # Create payload
    payload = {
        "name": sr_name,
        "offering_title": "Access Management - Uprising - Create New User Account",
        "source_keyword": "MYIT",
        ...
        "sr_type_field_19": u_manager_email,
        "sr_type_field_20": u_env,
        "sr_type_field_21": u_platform,
        "sr_type_field_22": u_description,
    ...
    logger.debug(f"Service Request Creation URL: {create_servie_request_url} and payload: {json.dumps(payload, indent=2)}")
...
def sr_worklog_update_new_user_account(**context) -> Dict[str, Any]:
    """
    Update worklog with service request information for new user account.
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting worklog update for new user account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    if not activity_data or 'data' not in activity_data or 'sr_creation_result' not in activity_data['data']:
        error_msg = "No SR creation result found in activity data"
    ...
    hook.info(f"Processing worklog update for SR: {request_number}")
    ...
    try:
        with httpx.Client(verify=False, timeout=480.0) as client:
            ...
            if not instance_id:
                error_msg = "No instance ID found in service request data"
                hook.error(error_msg)
                activity_data['data']['work_info_result'] = {
                    'status': 'Failed',
                    'error': error_msg
            ...
            # Create work info
            work_info_payload = {
                "name": f"New User Account Work Info {request_number}",
            ...
            logger.debug(f"Created work info with ID: {work_info_data.get('id')}")
...
def sr_creation_activity_update_new_user_account(**context) -> Dict[str, Any]:
    """
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting activity update for new user account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.sr_worklog_update_new_user_account', key='activity_data')
...
def fetch_itsm_approver_new_user_account(**context) -> Dict[str, Any]:
    """
    Fetch ITSM approver information for new user account.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'approver_result': {
                'approver_names': ['approver1', 'approver2'],
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.sr_creation_activity_update_new_user_account', key='activity_data')
    ...
    """
    logger.debug(f"Fetch ITSM approve list, approver name Query: {query}")
    ...
    try:
        ...
        if ';' in approver_list_str:
            ...
            approver_list = approver_list_str.split(',')
        approver_list = [adid.strip() for adid in approver_list if adid.strip()]  # Clean up and remove empty entries
        ...
        if ';' in approver_names_str:
            ...
            approver_names = approver_names_str.split(',')
        approver_names = [name.strip() for name in approver_names if name.strip()]  # Clean up and remove empty entries
        ...
        for adid in approver_list:
            ...
            """
            logger.debug(f"Email query for adid {adid.strip()}: {email_query}")
...
def send_communication_sr_creation_new_user_account(**context) -> Dict[str, Any]:
    """
    Send communication for service request creation for new user account.
    ...
    Example input (from activity_data):
    {
        'data': {
            ...
            'approver_result': {
                'approver_names': ['approver full name 1','approver full name 2'],
                'approver_emails': ['approver1@company.com', 'approver2@company.com']
            }
        }
    }
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.fetch_itsm_approver_new_user_account', key='activity_data')
    if not activity_data or 'data' not in activity_data:
        error_msg = "No activity data found"
        hook.error(error_msg)
        return {
    ...
    approver_result = activity_data['data'].get('approver_result', {})

    # Get user and manager info with null checks
    ...
    # Get SR info with null check
    sr_number = sr_result.get('request_number')

    # Get other activity info with null checks
    ...
    task_type = activity_info.get('task_type', 'New User Account')

    # Get approver info with null checks
    approver_names = approver_result.get('approver_names', [])
    approver_emails = approver_result.get('approver_emails', [])
    ...
    # Create comma-separated string of approver names for email
    approver_names_for_email = ', '.join(approver_names) if approver_names else "No approvers assigned"

    # Get email group with null check
    email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")
    if not email_group:
        hook.warning("No email group found in environment variables, using default")
        email_group = "am_email_group"

    # Construct cc_notifier_email_list with null checks
    cc_notifier_email_list = [email_group]
    if manager_email:
        cc_notifier_email_list.append(manager_email)
    if approver_emails:
        cc_notifier_email_list.extend(approver_emails)

    process_id = activity_data['data'].get('process_id')
    if not process_id:
        error_msg = "Process ID is missing"
        hook.error(error_msg)
        return {
            "success": False,
            "error": error_msg
    ...
    try:
        ...
        # Fetch template using mapping hook sr_creation_email_template_new_user_account
        mapping_elements = mapping_hook.list_mapping_elements(
            ...
            mapping_key="sr_creation_email_template_new_user_account"
        )
        ...
        subject = f"Access Management | Create User Account | Process Id: {process_id}"
...
# AD process success flow when user already exists in AD
def sr_creation_activity_update_ad_process_user_exists_error_new_user_account(**context) -> Dict[str, Any]:
    """
    Update activity status when AD process fails during new user account creation.
    ...
    Example input (from activity_data):
    {
        'data': {
            ...
            'ad_process_result': {
                ...
                'AD_OUTPUT': 'User already exists in AD'
            }
        }
    }
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'activity_update_result_ad_process_user_exists_error': {
                'status': 'Completed',
                'remarks': 'The user name is already present in AD.',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.send_communication_ad_process_existing_user_error_new_user_account', key='activity_data')
    if not activity_data or 'data' not in activity_data:
        error_msg = "No activity data found"
        hook.error(error_msg)
        return {
    ...
    update_query = f"""
        ...
        SET u_status = 'Completed',
            u_remarks = 'The user name is already present in AD.'
        WHERE u_identifier = '{process_id}'
    """
...
def send_communication_ad_process_existing_user_error_new_user_account(**context) -> Dict[str, Any]:
    """
    Send communication for AD process error when user already exists in new user account creation.
    ...
    Example input (from activity_data):
    {
        'data': {
            ...
            'activity_update_result_ad_process_error': {
                ...
                'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New User Account)',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'email_notification_result_ad_process_existing_user_error_new_user_account': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'user_adid': 'JDOE123',
                'timestamp': '2024-03-21T17:31:05.948'
    ...
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for AD process existing user error")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
    ...
    hook.info(f"Sending AD process existing user error notification to user: {user_email}")
    ...
    try:
        ...
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_sr_management",
            mapping_namespace_name="access_management_enable",
            mapping_key="ad_process_existing_user_error_email_template_new_user_account"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for AD process existing user error template")
        ...
        try:
            ...
            # Update activity_data with success information
            activity_data['data']['email_notification_result_ad_process_existing_user_error_new_user_account'] = {
                'status': 'Sent',
                'recipients': [user_email],
                'timestamp': datetime.now().isoformat()
            }
...
def sr_creation_activity_update_ad_process_errored_new_user_account(**context) -> Dict[str, Any]:
    """
    Update activity status when AD process fails during new user account creation.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'activity_update_result_ad_process_error': {
                ...
                'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request New User Account Creation',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    ...
    update_query = f"""
        ...
        SET u_status = 'Error in check AD process',
            u_remarks = 'Transfer to FO team. ITSM error in SR submission. Validation successful for request New User Account Creation'
        WHERE u_identifier = '{process_id}'
    """
...
def send_communication_ad_process_errored_new_user_account(**context) -> Dict[str, Any]:
    """
    Send communication for AD process error during new user account creation.
    ...
    Example output (added to activity_data):
    {
        'data': {
            ...
            'email_notification_result_ad_process_error_new_user_account': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'user_adid': 'JDOE123',
                'timestamp': '2024-03-21T17:31:05.948'
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.sr_creation_activity_update_ad_process_errored_new_user_account', key='activity_data')
    activity_info = activity_data['data'].get('activity_info_fetched', {})
    ...
    try:
        ...
        try:
            ...
            # Update activity_data with success information
            activity_data['data']['email_notification_result_ad_process_error_new_user_account'] = {
                'status': 'Sent',
                'recipients': [user_email],
                'timestamp': datetime.now().isoformat()
            }
...
def sr_creation_failed_activity_update_new_user_account(**context) -> Dict[str, Any]:
    """
    Update activity status when SR creation fails for new user account.
    ...
    try:
        ...
        query = f"""
            ...
            SET u_status = 'Error in ITSM Process',
                u_remarks = 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New User Account)'
            WHERE u_identifier = '{process_id}'
        """
        try:
            # Execute query
            ...
            return {
                ...
                "data": {
                    "process_id": process_id,
                    "status": "Error in ITSM Process",
                    "message": "Transfer to FO team. ITSM error in SR submission. Validation successful for request (New User Account)",
                    "timestamp": datetime.now().isoformat()
                }
            }
...
def sr_creation_failed_email_notification_new_user_account(**context) -> Dict[str, Any]:
    """
    Send email notification when SR creation fails for new user account.
    ...
    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_user_account_group.sr_creation_failed_activity_update_new_user_account', key='activity_data')
    user_email = activity_data['data']['activity_info_fetched']['user_email']
    user_name = activity_data['data']['activity_info_fetched']['user_name']
