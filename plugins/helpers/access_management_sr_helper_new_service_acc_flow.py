import logging
from typing import Dict, Any
from hooks.access_management_db_hook import AccessManagementSQLHook
from hooks.ad_process_hook import AD_ProcessHook
from airflow.sdk.bases.hook import BaseHook
from hooks.worklog_hook import WorkLogHook
import uuid
import os
from datetime import datetime
from hooks.tasks_hook import TasksHook
import httpx
from jinja2 import Template
from hooks.mapping_hook import MappingHook
from hooks.notify_hook import NotifyHook

logger = logging.getLogger(__name__)

def check_service_account_name_new_service_account(**context) -> Dict[str, Any]:
    """
    Fetch activity information for new_service_account from EXT_ACCESS_MANAGEMENT_ACTIVITY table.

    This function:
    1. Gets activity_data from XCom
    2. Queries EXT_ACCESS_MANAGEMENT_ACTIVITY table for user details
    3. Updates activity_data with fetched information
    4. Returns updated activity_data

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345'
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            }
        }
    }

    Example error output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name_creation_failed': 'Error message here'
            }
        }
    }
    """
    ti = context['ti']
    activity_data = ti.xcom_pull(task_ids='update_or_create_request_activity', key='activity_data')
    activity_identifier = activity_data['data']['process_id']

    try:
        # Get connection id from environment variable
        oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

        # Get worklog_id from XCom
        worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)

        # Get database connection
        db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

        # Fetch service account details from activity table
        query = f"""
            SELECT u_svc_name, u_description
            FROM EXT_ACCESS_MANAGEMENT_ACTIVITY
            WHERE u_identifier = '{activity_identifier}'
        """
        result = db_hook.get_row_data(query)
        if not result.get('success') or not result.get('data'):
            error_msg = f"No activity found for process_id: {activity_identifier}"
            hook.error(error_msg)
            if 'service_account_name_check' not in activity_data['data']:
                activity_data['data']['service_account_name_check'] = {}
            activity_data['data']['service_account_name_check']['svc_name_creation_failed'] = error_msg
            ti.xcom_push(key='activity_data', value=activity_data)
            return activity_data

        service_account_name = result['data'][0] if result['data'] else ''
        description = result['data'][1] if result['data'] else ''

        # Check service account name length
        if len(service_account_name) > 20:
            error_msg = f"Service account name '{service_account_name}' exceeds 20 characters"
            hook.error(error_msg)
            if 'service_account_name_check' not in activity_data['data']:
                activity_data['data']['service_account_name_check'] = {}
            activity_data['data']['service_account_name_check']['svc_name_creation_failed'] = error_msg
            ti.xcom_push(key='activity_data', value=activity_data)
            return activity_data

        # Process service account name based on conditions
        new_svc_name = service_account_name
        if not service_account_name.startswith('svc_'):
            if any(db_term in description.lower() for db_term in ['db', 'database']):
                new_svc_name = f"db_{service_account_name}"
            else:
                new_svc_name = f"svc_{service_account_name}"

            # Update the service account name in database
            update_query = f"""
                UPDATE EXT_ACCESS_MANAGEMENT_ACTIVITY 
                SET u_svc_name = '{new_svc_name}'
                WHERE u_identifier = '{activity_identifier}'
            """
            update_result = db_hook.execute_query(update_query)
            if not update_result.get('success'):
                error_msg = f"Failed to update service account name in database: {update_result.get('error', 'Unknown error')}"
                hook.error(error_msg)
                if 'service_account_name_check' not in activity_data['data']:
                    activity_data['data']['service_account_name_check'] = {}
                activity_data['data']['service_account_name_check']['svc_name_creation_failed'] = error_msg
                ti.xcom_push(key='activity_data', value=activity_data)
                return activity_data

        # Add service account name check info to activity data
        if 'service_account_name_check' not in activity_data['data']:
            activity_data['data']['service_account_name_check'] = {}

        activity_data['data']['service_account_name_check']['svc_name'] = new_svc_name
        ti.xcom_push(key='activity_data', value=activity_data)
        return activity_data

    except Exception as e:
        error_msg = f"Error processing service account name: {str(e)}"
        hook.error(error_msg)
        if 'service_account_name_check' not in activity_data['data']:
            activity_data['data']['service_account_name_check'] = {}
        activity_data['data']['service_account_name_check']['svc_name_creation_failed'] = error_msg
        # Remove svc_name if it was set before the error
        activity_data['data']['service_account_name_check'].pop('svc_name', None)
        ti.xcom_push(key='activity_data', value=activity_data)
        return activity_data


def fetch_data_activity_info_new_service_account(**context) -> Dict[str, Any]:
    """
    Fetch activity information for new service account creation from EXT_ACCESS_MANAGEMENT_ACTIVITY table.

    This function:
    1. Gets activity_data from XCom
    2. Queries EXT_ACCESS_MANAGEMENT_ACTIVITY table for request details
    3. Updates activity_data with fetched information
    4. Returns updated activity_data

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            }
        }
    }
    """
    ti = context['ti']
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.check_service_account_name_new_service_account', key='activity_data')
    activity_identifier = activity_data['data']['process_id']

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    hook = WorkLogHook()
    hook.set_worklog_id(worklog_id)

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to fetch required fields
    query = f"""
        SELECT
            u_task_type,
            u_user_name,
            u_user_email,
            u_manager_name,
            u_manager_email,
            u_env,
            u_description,
            u_organization,
            u_platform,
            u_business_justification,
            u_role
        FROM EXT_ACCESS_MANAGEMENT_ACTIVITY
        WHERE u_identifier = '{activity_identifier}'
    """

    # Execute query
    result = db_hook.get_row_data(query)

    if not result.get('success') or not result.get('data'):
        hook.error(f"No activity found for process_id: {activity_identifier}")
        raise ValueError(f"No activity found for process_id: {activity_identifier}")

    # Add activity info to existing activity_data
    activity_data['data']['activity_info_fetched'] = {
        'task_type': result['data'][0],      # u_task_type
        'user_name': result['data'][1],      # u_user_name
        'user_email': result['data'][2],     # u_user_email
        'manager_name': result['data'][3],   # u_manager_name
        'manager_email': result['data'][4],  # u_manager_email
        'env': result['data'][5],           # u_env
        'description': result['data'][6],    # u_description
        'organization': result['data'][7],   # u_organization
        'platform': result['data'][8],       # u_platform
        'business_justification': result['data'][9],  # u_business_justification
        'role': result['data'][10]           # u_role
    }

    hook.info(f"Fetched activity info for process_id: {activity_identifier}")

    # Push updated activity_data back to XCom
    ti.xcom_push(key='activity_data', value=activity_data)

    return activity_data

def ad_process_new_service_account(**context) -> Dict[str, Any]:
    """
    Connect to AD and execute new service account creation process.

    This function:
    1. Gets service account details from activity_data
    2. Gets worklog_id from XCom
    3. Prepares and executes AD process using AD_ProcessHook
    4. Updates activity_data with the process result

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Created Successfully',
                'ad_output_new_service_account': 'Service Account Created Successfully'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting AD new service account creation process")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.fetch_data_activity_info_new_service_account', key='activity_data')
    svc_name = activity_data['data']['service_account_name_check']['svc_name']
    process_id = activity_data['data']['process_id']
    user_name = activity_data['data']['activity_info_fetched']['user_name']
    user_email = activity_data['data']['activity_info_fetched']['user_email']
    task_type = activity_data['data']['activity_info_fetched']['task_type']
    manager_name = activity_data['data']['activity_info_fetched']['manager_name']
    manager_email = activity_data['data']['activity_info_fetched']['manager_email']
    env = activity_data['data']['activity_info_fetched']['env']
    description = activity_data['data']['activity_info_fetched']['description']
    organization = activity_data['data']['activity_info_fetched']['organization']
    platform = activity_data['data']['activity_info_fetched']['platform']
    organization = activity_data['data']['activity_info_fetched']['organization']
    business_justification = activity_data['data']['activity_info_fetched']['business_justification']
    role = activity_data['data']['activity_info_fetched']['role']

    hook.info(f"Processing new service account creation for: {svc_name}")

    # Get connection details from Airflow with fallback values
    try:
        # Choose connection based on environment
        if 'prod' not in env.lower():
            conn = BaseHook.get_connection('ad_process_conn_lab')
            fallback_host = '10.253.228.29'
            fallback_user = 'm2m_enable_auto@lab.uprising.t-mobile.com'
            fallback_password = 'VBR41w6a5vc88S9tKJ7mFQ4pZLXT2V'
        else:
            conn = BaseHook.get_connection('ad_process_conn')
            fallback_host = '10.159.176.105'
            fallback_user = 'm2m_enable_auto@uprising.t-mobile.com'
            fallback_password = 'VBR41w6a5vc88S9tKJ7mFQ4pZLXT2V'
        ssh_host = conn.host or fallback_host
        ssh_port = conn.port or 22
        ssh_user = conn.login or fallback_user
        ssh_password = conn.password or fallback_password
    except Exception as e:
        hook.warning(f"Failed to get connection details from Airflow, using default values: {str(e)}")
        # Set fallback host based on environment
        fallback_host = '10.253.228.29' if 'prod' not in env.lower() else '10.159.176.105'
        ssh_host = fallback_host
        ssh_port = 22
        ssh_user = 'm2m_enable_auto@uprising.t-mobile.com'
        ssh_password = 'VBR41w6a5vc88S9tKJ7mFQ4pZLXT2V'

    hook.info("Creating input file for new service account creation")
    # Create input file command for new service account
    input_command = f'echo {{"inputvalues": [{{"SVC_ADID" : "{svc_name}"}}],"action_name" : "Check Service Account"}} > "C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\checkSvcAcc\\checksvcacc_i_{process_id}.json"'
    # Example: echo {"inputvalues": [{"SVC_ADID" : "$FLOW{SVC_NAME}"}],"action_name" : "Check Service Account"}  >  "C:\Users\m2m_enable_auto\Documents\Access Management Automation\New Svc Account Creation\checkSvcAcc\checksvcacc_i_$PARAM{IDENTIFIER}.json"

    # Script command to execute the batch file
    script_cmd = '"C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\check_svc_account.bat"'
    # "C:\Users\m2m_enable_auto\Documents\Access Management Automation\New Svc Account Creation\check_svc_account.bat"

    # Output file paths
    output_file = f'C:\\Users\\m2m_enable_auto\\Documents\\Access Management Automation\\New Svc Account Creation\\CheckSvcAcc\\checksvcacc_o_{process_id}.json'
    # Example: C:\Users\m2m_enable_auto\Documents\Access Management Automation\New Svc Account Creation\CheckSvcAcc\checksvcacc_o_$PARAM{IDENTIFIER}.json

    target_scp_file_path = 'm2m_enable_auto@uprising.t-mobile.com@10.253.29.123:/enable/Access_Management'

    hook.info("Executing AD process")
    # Create and execute hook
    with AD_ProcessHook(
        ssh_host=ssh_host,
        ssh_user=ssh_user,
        ssh_password=ssh_password,
        ssh_port=ssh_port,
        input_command=input_command,
        script_cmd=script_cmd,
        output_file=output_file,
        target_scp_file_path=target_scp_file_path,
        scp_target='m2m_enable_auto@uprising.t-mobile.com@10.253.29.123:/enable/Access_Management',
        scp_password='VBR41w6a5vc88S9tKJ7mFQ4pZLXT2V',
        worklog_id=worklog_id
    ) as ad_process_hook:
        result = ad_process_hook.execute_process()

    if result['success']:
        hook.info("New service account creation process completed successfully")
        # Add the AD process result to activity_data
        activity_data['data']['ad_process_result'] = result['data']
        # Store AD output in new field for next step
        activity_data['data']['ad_output_new_service_account'] = result['data']['AD_OUTPUT']
        ti.xcom_push(key='activity_data', value=activity_data)
    else:
        hook.error(f"New service account creation process failed: {result.get('error', 'Unknown error')}")
        # Add the AD process result to activity_data even in error case
        activity_data['data']['ad_process_result'] = result['data']
        # Store AD output in new field for next step
        activity_data['data']['ad_output_new_service_account'] = result['data'].get('AD_OUTPUT', '')
        ti.xcom_push(key='activity_data', value=activity_data)

    return result

def sr_creation_new_service_account(**context) -> Dict[str, Any]:
    """
    Create service request for new service account.
    This function:
    1. Gets worklog_id from XCom
    2. Gets service account details from activity_data
    3. Creates service request for new service account
    4. Updates activity_data with SR details

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Created Successfully'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'Password Reset'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Created Successfully'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting SR creation for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.ad_process_new_service_account', key='activity_data')
    process_id = activity_data['data']['process_id']
    svc_name = activity_data['data']['service_account_name_check']['svc_name']

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    hook.info(f"Creating SR for new service account: {svc_name}")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to fetch activity data
    query = f"""
        SELECT
            u_business_justification,
            u_user_email,
            u_manager_name,
            u_manager_adid,
            u_svc_owner_email,
            u_manager_email,
            u_user_name,
            u_organization,
            u_svc_name,
            u_intended_use,
            u_user_adid,
            u_env,
            u_platform,
            u_description,
            u_mobile_no,
            u_organization,
            u_platform
        FROM EXT_ACCESS_MANAGEMENT_ACTIVITY
        WHERE u_identifier = '{process_id}'
    """

    # Execute query
    result = db_hook.get_row_data(query)

    if not result.get('success') or not result.get('data'):
        error_msg = f"No activity found for process_id: {process_id}"
        hook.error(error_msg)
        activity_data['data']['sr_creation_result'] = {
            'status': 'Failed',
            'error': error_msg
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

    # Get the activity data
    activity_row = result['data']
    hook.info(f"Fetched activity data for process_id: {process_id}")

    # Extract fields from activity_row with explicit field names
    u_business_justification = activity_row[0]  # u_business_justification
    u_user_email = activity_row[1]  # u_user_email
    u_manager_name = activity_row[2]  # u_manager_name
    u_manager_adid = activity_row[3]  # u_manager_adid
    u_svc_owner_email = activity_row[4]  # u_svc_owner_email
    u_manager_email = activity_row[5]  # u_manager_email
    u_user_name = activity_row[6]  # u_user_name
    u_organization = activity_row[7]  # u_organization
    u_svc_name = activity_row[8]  # u_svc_name
    u_intended_use = activity_row[9]  # u_intended_use
    u_user_adid = activity_row[10]  # u_user_adid
    u_env = activity_row[11]  # u_env
    u_platform = activity_row[12]  # u_platform
    u_description = activity_row[13]  # u_description
    u_mobile_no = activity_row[14]  # u_mobile_no
    u_organization = activity_row[15]  # u_organization
    u_platform = activity_row[16]  # u_platform

    # Fetch approver based on organization and platform
    approver = None
    approver_query = f"""
        SELECT u_approver_list
        FROM EXT_AM_ITSM_APPROVER_ROLE 
        WHERE u_company = '{u_organization}'
        AND u_platform = '{u_platform}'
    """
    approver_result = db_hook.get_row_data(approver_query)
    if approver_result.get('success') and approver_result.get('data'):
        approver = approver_result['data'][0]  # Get first column from tuple
        hook.info(f"Fetched approver: {approver}")
    else:
        hook.warning("No approver found, using empty string")
        approver = ""

    # Create formatted notes string
    notes = (
        f"Service Account Name - {u_svc_name};\n"
        f"User Name - {u_user_name};\n"
        f"User Email - {u_user_email};\n"
        f"Manager Name - {u_manager_name};\n"
        f"Manager Email - {u_manager_email};\n"
        f"Manager AD ID - {u_manager_adid};\n"
        f"Service Owner Email - {u_svc_owner_email};\n"
        f"Environment - {u_env};\n"
        f"Description - {u_description};\n"
        f"Organization - {u_organization};\n"
        f"Platform - {u_platform};\n"
        f"Business Justification - {u_business_justification};\n"
        f"Intended Use - {u_intended_use};\n"
        f"User AD ID - {u_user_adid};\n"
        f"Mobile Number - {u_mobile_no};\n"
        f"Approver - {approver}"
    )

    # Add notes to activity data under sr_creation_result
    activity_data['data']['sr_creation_result'] = {'notes': notes}
    ti.xcom_push(key='activity_data', value=activity_data)

    hook.info(f"Added formatted notes to activity data")

    MIDDLEWARE_HOST: str = os.environ.get("MIDDLEWARE_HOST", "middleware")
    MIDDLEWARE_PORT: int = int(os.environ.get("MIDDLEWARE_PORT", 5200))
    ticketing_system: str = os.environ.get("DEFAULT_TICKETING_SYSTEM", "REMEDY")

    create_servie_request_url = f"https://{MIDDLEWARE_HOST}:{MIDDLEWARE_PORT}/api/v1/tickets/service-requests?system={ticketing_system}"

    sr_name = f"New Service Account Creation - {u_svc_name} ({u_user_name}) - {u_organization}"
    # Prepare the request payload
    payload = {
        "name": sr_name,
        "offering_title": "Access Management - Uprising - Create New Service Account",
        "source_keyword": "MYIT",
        "title_instance_id": "SRHAA5V0G9R60AO3DGHV0JAULUI5MK",
        "full_name": "Svc Enable Automation",
        "login_id": "svc_enable_itsm",
        "sr_type_field_1": u_business_justification,
        "sr_type_field_2": u_user_email,
        "sr_type_field_3": "Access Management User Create",
        "sr_type_field_4": u_manager_name,
        "sr_type_field_5": u_manager_adid.lower(),
        "sr_type_field_10": approver,
        "sr_type_field_11": u_svc_owner_email,
        "sr_type_field_12": u_manager_email,
        "sr_type_field_13": u_user_name,
        "sr_type_field_16": u_organization,
        "sr_type_field_17": u_svc_name,
        "sr_type_field_19": u_user_adid,
        "sr_type_field_20": u_env,
        "sr_type_field_21": u_platform,
        "sr_type_field_22": u_description[:80] if len(u_description) > 80 else u_description,
        "sr_type_field_42": u_mobile_no,
        "sr_type_field_43": "Add"
    }

    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json"
    }

    try:
        with httpx.Client(verify=False, timeout=480.0) as client:
            response = client.post(
                create_servie_request_url,
                json=payload,
                headers=headers
            )
            response.raise_for_status()

            sr_data = response.json()
            hook.info(f"SR creation response: {sr_data}")

            # Validate response structure
            if not isinstance(sr_data, dict):
                error_msg = "Invalid response format: expected dictionary"
                hook.error(error_msg)
                activity_data['data']['sr_creation_result'] = {
                    'status': 'Failed',
                    'error': error_msg
                }
                ti.xcom_push(key='activity_data', value=activity_data)
                return {
                    "success": False,
                    "error": error_msg
                }

            # Validate required fields
            required_fields = ['request_number', 'status']
            missing_fields = [field for field in required_fields if field not in sr_data]
            if missing_fields:
                error_msg = f"Missing required fields in response: {missing_fields}"
                hook.error(error_msg)
                activity_data['data']['sr_creation_result'] = {
                    'status': 'Failed',
                    'error': error_msg
                }
                ti.xcom_push(key='activity_data', value=activity_data)
                return {
                    "success": False,
                    "error": error_msg
                }

            sr_result = {
                'request_number': sr_data['request_number'],
                'status': sr_data['status']
            }

            hook.info(f"Successfully created SR: {sr_result}")

            # Update activity_data with SR creation result
            activity_data['data']['sr_creation_result'] = sr_result
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": sr_result
            }

    except httpx.HTTPError as e:
        error_msg = f"HTTP error occurred: {str(e)}"
        hook.error(error_msg)
        activity_data['data']['sr_creation_result'] = {
            'status': 'Failed',
            'error': error_msg
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }
    except Exception as e:
        error_msg = f"Error creating service request: {str(e)}"
        hook.error(error_msg)
        activity_data['data']['sr_creation_result'] = {
            'status': 'Failed',
            'error': error_msg
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def sr_worklog_update_new_service_account(**context) -> Dict[str, Any]:
    """
    Update worklog with service request information for new service account.

    This function:
    1. Gets SR ID from activity_data
    2. Searches for the service request using the API
    3. Gets the instance ID from the response
    4. Creates work info using the instance ID
    5. Updates activity_data with work info details

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            },
            'work_info_result': {
                'work_info_id': 'WI1234',
                'status': 'Created'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting worklog update for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.sr_creation_new_service_account', key='activity_data')
    request_number = activity_data['data']['sr_creation_result']['request_number']

    hook.info(f"Searching for service request with number: {request_number}")

    # Search for service request
    MIDDLEWARE_HOST: str = os.environ.get("MIDDLEWARE_HOST", "middleware")
    MIDDLEWARE_PORT: int = int(os.environ.get("MIDDLEWARE_PORT", 5200))
    ticketing_system: str = os.environ.get("DEFAULT_TICKETING_SYSTEM", "REMEDY")
    search_url = f"https://{MIDDLEWARE_HOST}:{MIDDLEWARE_PORT}/api/v1/tickets/service-requests?request_number={request_number}&system={ticketing_system}"

    try:
        with httpx.Client(verify=False, timeout=480.0) as client:
            response = client.get(search_url)
            response.raise_for_status()
            sr_data = response.json()

            if not sr_data or not isinstance(sr_data, list) or len(sr_data) == 0:
                error_msg = f"No service request found with number: {request_number}"
                hook.error(error_msg)
                activity_data['data']['work_info_result'] = {
                    'status': 'Failed',
                    'error': error_msg
                }
                ti.xcom_push(key='activity_data', value=activity_data)
                return {
                    "success": False,
                    "error": error_msg
                }

            # Get the first matching service request
            sr = sr_data[0]
            instance_id = sr.get('instance')

            if not instance_id:
                error_msg = "No instance ID found in service request response"
                hook.error(error_msg)
                activity_data['data']['work_info_result'] = {
                    'status': 'Failed',
                    'error': error_msg
                }
                ti.xcom_push(key='activity_data', value=activity_data)
                return {
                    "success": False,
                    "error": error_msg
                }

            hook.info(f"Found service request with instance ID: {instance_id}")

            # Create work info
            work_info_url = f"https://{MIDDLEWARE_HOST}:{MIDDLEWARE_PORT}/api/v1/tickets/service-requests/work-info?system={ticketing_system}"
            work_info_payload = {
                "name": f"New Service Account Work Info {request_number}",
                "notes": activity_data['data']['sr_creation_result']['notes'],
                "summary": "Request Information",
                "work_info_type_selection": "General Information",
                "sr_instance_id": instance_id,
                "service_request_instance_id": instance_id,
                "request_number": request_number,
                "service_request_number": request_number,
                "sr_id": request_number,
                "secure_log": "Yes",
                "view_access": "Public",
                "work_info_type": "General Information",
            }

            work_info_response = client.post(work_info_url, json=work_info_payload)
            work_info_response.raise_for_status()
            work_info_data = work_info_response.json()

            hook.info(f"Created work info with ID: {work_info_data.get('id')}")

            # Update activity_data with work info result
            activity_data['data']['work_info_result'] = {
                'work_info_id': work_info_data.get('id'),
                'status': 'Created'
            }
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": work_info_data
            }

    except httpx.HTTPError as e:
        error_msg = f"HTTP error occurred: {str(e)}"
        hook.error(error_msg)
        activity_data['data']['work_info_result'] = {
            'status': 'Failed',
            'error': error_msg
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }
    except Exception as e:
        error_msg = f"Error occurred: {str(e)}"
        hook.error(error_msg)
        activity_data['data']['work_info_result'] = {
            'status': 'Failed',
            'error': error_msg
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def sr_creation_activity_update_new_service_account(**context) -> Dict[str, Any]:
    """
    Update EXT_ACCESS_MANAGEMENT_ACTIVITY table with SR number and status.

    This function:
    1. Gets worklog_id from XCom
    2. Gets SR number and process_id from activity_data
    3. Updates EXT_ACCESS_MANAGEMENT_ACTIVITY table with SR number and status

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            },
            'work_info_result': {
                'work_info_id': 'WI1234',
                'status': 'Created'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            },
            'work_info_result': {
                'work_info_id': 'WI1234',
                'status': 'Created'
            },
            'activity_update_result': {
                'status': 'Completed',
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting activity table update for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.sr_worklog_update_new_service_account', key='activity_data')
    process_id = activity_data['data']['process_id']
    sr_number = activity_data['data']['sr_creation_result']['request_number']

    hook.info(f"Updating activity table for process_id: {process_id} with SR number: {sr_number}")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to update activity table
    query = f"""
        UPDATE EXT_ACCESS_MANAGEMENT_ACTIVITY
        SET u_sr_no = '{sr_number}',
            u_status = 'SR Submitted'
        WHERE u_identifier = '{process_id}'
    """

    try:
        # Execute query
        result = db_hook.execute_query(query)

        if not result.get('success'):
            error_msg = result.get('error', 'Unknown error')
            hook.error(f"Failed to update activity table: {error_msg}")
            # Update activity_data with error information
            activity_data['data']['activity_update_result'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        hook.info(f"Successfully updated activity table for process_id: {process_id}")

        # Update activity_data with success information
        activity_data['data']['activity_update_result'] = {
            'status': 'Completed',
            'sr_number': sr_number,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)

        return {
            "success": True,
            "data": {
                "status": "Completed",
                "sr_number": sr_number
            }
        }

    except Exception as e:
        error_msg = f"Error updating activity table: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['activity_update_result'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def fetch_itsm_approver_new_service_account(**context) -> Dict[str, Any]:
    """
    Fetch ITSM approver information for new service account.

    This function:
    1. Gets worklog_id from XCom
    2. Gets company, description, and platform from activity_data
    3. Queries EXT_AM_ITSM_APPROVER_ROLE for approver list
    4. Queries EXT_AM_ITSM_USER_LIST for approver emails
    5. Updates activity_data with approver information

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'activity_update_result': {
                'status': 'SR Submitted',
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'activity_update_result': {
                'status': 'SR Submitted',
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'approver_result': {
                'approver_names': ['John Approver', 'Jane Approver'],  # List of names
                'approver_emails': ['approver1@company.com', 'approver2@company.com'],  # List of emails
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting ITSM approver fetch for new service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.sr_creation_activity_update_new_service_account', key='activity_data')
    company = activity_data['data']['activity_info_fetched']['organization']
    description = activity_data['data']['activity_info_fetched']['description']
    platform = activity_data['data']['activity_info_fetched']['platform']

    hook.info(f"Fetching approvers for company: {company}, description: {description}, platform: {platform}")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # First query to get approver list and name
    query = f"""
        SELECT u_approver_list, u_approver_name
        FROM EXT_AM_ITSM_APPROVER_ROLE
        WHERE u_company = '{company}'
        AND (u_role_des = '{description}' OR u_platform = '{platform}')
    """

    try:
        # Execute first query
        result = db_hook.get_row_data(query)

        if not result.get('success') or not result.get('data'):
            error_msg = f"No approvers found for company: {company}, description: {description}, platform: {platform}"
            hook.error(error_msg)
            activity_data['data']['approver_result'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        # Get approver list and name from result
        approver_list_str = result['data'][0]
        approver_names_str = result['data'][1]

        # Handle both semicolon and comma delimiters for approver list
        if ';' in approver_list_str:
            approver_list = approver_list_str.split(';')
        else:
            approver_list = approver_list_str.split(',')
        approver_list = [adid.strip() for adid in approver_list if adid.strip()]

        # Handle both semicolon and comma delimiters for approver names
        if ';' in approver_names_str:
            approver_names = approver_names_str.split(';')
        else:
            approver_names = approver_names_str.split(',')
        approver_names = [name.strip() for name in approver_names if name.strip()]

        hook.info(f"Found approver list: {approver_list}")
        hook.info(f"Found approver names: {approver_names}")

        # Get emails one by one
        approver_emails = []
        for adid in approver_list:
            if not adid.strip():
                continue
            email_query = f"""
                SELECT DISTINCT u_email
                FROM EXT_AM_ITSM_USER_LIST
                WHERE u_login_id = '{adid.strip()}'
            """
            email_result = db_hook.get_row_data(email_query)
            if email_result.get('success') and email_result.get('data'):
                approver_emails.append(email_result['data'][0])

        if not approver_emails:
            error_msg = "No email addresses found for approvers"
            hook.error(error_msg)
            activity_data['data']['approver_result'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        hook.info(f"Found approver emails: {approver_emails}")

        # Update activity_data with approver information
        activity_data['data']['approver_result'] = {
            'approver_names': approver_names,  # List of names
            'approver_emails': approver_emails,  # List of emails
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)

        return {
            "success": True,
            "data": {
                "approver_names": approver_names,  # List of names
                "approver_emails": approver_emails  # List of emails
            }
        }

    except Exception as e:
        error_msg = f"Error fetching approver information: {str(e)}"
        hook.error(error_msg)
        activity_data['data']['approver_result'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def send_communication_sr_creation_new_service_account(**context) -> Dict[str, Any]:
    """
    Send communication for service request creation.

    This function:
    1. Gets worklog_id from XCom
    2. Gets user data and SR details from activity_data
    3. Prepares email content using template from mapping
    4. Sends email notification using email hook

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            },
            'activity_update_result': {
                'status': 'SR Submitted',
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'approver_result': {
                'approver_list': ['APPROVER1', 'APPROVER2'],
                'approver_name': 'Approver Group Name',
                'approver_emails': ['approver1@company.com', 'approver2@company.com']
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'request_number': 'REQ1234',
                'status': 'Assigned',
                'notes': 'Service Account Name - svc_testaccount;\nUser Name - John Doe;\nUser Email - john.doe@company.com;\nManager Name - Jane Smith;\nManager Email - jane.smith@company.com;\nEnvironment - production;\nDescription - Service account for database access;\nOrganization - IT;\nPlatform - Windows;\nBusiness Justification - Required for database maintenance;\nRole - Database Administrator'
            },
            'activity_update_result': {
                'status': 'SR Submitted',
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'approver_result': {
                'approver_list': ['APPROVER1', 'APPROVER2'],
                'approver_name': 'Approver Group Name',
                'approver_emails': ['approver1@company.com', 'approver2@company.com']
            },
            'email_notification_result': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'sr_number': 'REQ1234',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for service request creation")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.fetch_itsm_approver_new_service_account', key='activity_data')
    if not activity_data or 'data' not in activity_data:
        logger.error("No activity data found in XCom")
        return {"success": False, "error": "No activity data found in XCom"}

    info = activity_data['data'].get('activity_info_fetched', {})
    sr_result = activity_data['data'].get('sr_creation_result', {})
    approver_info = activity_data['data'].get('approver_result', {})

    user_email = info.get('user_email')
    user_name = info.get('user_name')
    svc_name = activity_data['data']['service_account_name_check'].get('svc_name', '')
    sr_number = sr_result.get('request_number', '')
    task_type = info.get('task_type', '')
    manager_email = info.get('manager_email')
    manager_name = info.get('manager_name')
    env = info.get('env')
    description = info.get('description')
    organization = info.get('organization')
    business_justification = info.get('business_justification')
    process_id = activity_data['data'].get('process_id', '')

    # Construct comma-separated approver_names string
    approver_names_str = ', '.join(approver_info.get('approver_names', [])) if approver_info.get('approver_names') else ''

    # Prepare template data
    template_data = {
        "task_type": task_type,
        "user_name": user_name,
        "svc_name": svc_name,
        "sr_number": sr_number,
        "env": env,
        "user_email": user_email,
        "manager_name": manager_name,
        "approver": approver_names_str,
        "description": description,
        "organization": organization,

        "business_justification": business_justification,

        "process_id": process_id
    }

    # Render content
    mapping_hook = MappingHook()
    mapping_elements = mapping_hook.list_mapping_elements(
        mapping_name="access_management_sr_management",
        mapping_namespace_name="access_management_enable",
        mapping_key="sr_creation_email_template_new_service_account"
    )
    if not mapping_elements:
        error_msg = "No mapping element found for service request creation template"
        logger.error(error_msg)
        return {"success": False, "error": error_msg}
    template_element = mapping_elements[0]
    if not isinstance(template_element, dict):
        error_msg = "Invalid mapping element format"
        logger.error(error_msg)
        return {"success": False, "error": error_msg}
    template_content = template_element.get("value")
    if not template_content:
        error_msg = "No template content found in mapping element"
        logger.error(error_msg)
        return {"success": False, "error": error_msg}
    rendered_content = Template(template_content).render(**template_data)

    subject = f"Access Management | Service Account Creation | Process Id: {process_id}"
    notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
    email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")

    # Build cc_notifier_id: email_group, manager_email, all approver_emails (no duplicates, all non-empty)
    cc_notifier_id = []
    if email_group:
        cc_notifier_id.append(email_group)
    if manager_email and manager_email not in cc_notifier_id:
        cc_notifier_id.append(manager_email)
    for email in approver_info.get('approver_emails', []):
        if email and email not in cc_notifier_id:
            cc_notifier_id.append(email)

    # Build notifier_id: user_email only (as in new_user_account)
    notifier_id = []
    if user_email:
        notifier_id.append(user_email)

    try:
        notify_hook = NotifyHook()
        result = notify_hook.send_notification({
            "name": notifier_name,
            "subject": subject,
            "message": rendered_content,
            "cc_notifier_id": cc_notifier_id,
            "notifier_id": notifier_id
        })
        hook.info("Send email notification task initiated")
        # Update activity_data with success information
        activity_data['data']['email_notification_result'] = {
            'status': 'Sent',
            'recipients': notifier_id + cc_notifier_id,
            'sr_number': sr_number,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {"success": True, "data": result}
    except Exception as e:
        error_msg = f"Failed to send email notification: {str(e)}"
        logger.error(error_msg)
        activity_data['data']['email_notification_result'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {"success": False, "error": error_msg}

def existing_service_account_activity_update_new_service_account(**context) -> Dict[str, Any]:
    """
    Update activity for existing service account.

    This function:
    1. Gets worklog_id from XCom
    2. Gets process_id from activity_data
    3. Updates EXT_ACCESS_MANAGEMENT_ACTIVITY table with status and remarks
    4. Updates activity_data with the update result

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Already Exists'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Already Exists'
            },
            'activity_update_result_existing_service_acc': {
                'status': 'Completed',
                'remarks': 'Service account already present',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting activity update for existing service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.ad_process_new_service_account', key='activity_data')
    process_id = activity_data['data']['process_id']
    svc_name = activity_data['data']['service_account_name_check']['svc_name']

    hook.info(f"Updating activity table for process_id: {process_id} - Service account {svc_name} already exists")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to update activity table
    query = f"""
        UPDATE EXT_ACCESS_MANAGEMENT_ACTIVITY
        SET u_status = 'Completed',
            u_remarks = 'Service account already present'
        WHERE u_identifier = '{process_id}'
    """

    try:
        # Execute query
        result = db_hook.execute_query(query)

        if not result.get('success'):
            error_msg = result.get('error', 'Unknown error')
            hook.error(f"Failed to update activity table: {error_msg}")
            # Update activity_data with error information
            activity_data['data']['activity_update_result_existing_service_acc'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        hook.info(f"Successfully updated activity table for process_id: {process_id}")

        # Update activity_data with success information
        activity_data['data']['activity_update_result_existing_service_acc'] = {
            'status': 'Completed',
            'remarks': 'Service account already present',
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)

        return {
            "success": True,
            "data": {
                "status": "Completed",
                "remarks": "Service account already present"
            }
        }

    except Exception as e:
        error_msg = f"Error updating activity table: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['activity_update_result_existing_service_acc'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def send_communication_existing_service_account_error_new_service_account(**context) -> Dict[str, Any]:
    """
    Send communication for existing service account error.

    This function:
    1. Gets worklog_id from XCom
    2. Gets user data and service account details from activity_data
    3. Prepares email content using template from mapping
    4. Sends email notification using email hook

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Already Exists'
            },
            'activity_update_result_existing_service_acc': {
                'status': 'Completed',
                'remarks': 'Service account already present',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished',
                'AD_OUTPUT': 'Service Account Already Exists'
            },
            'activity_update_result_existing_service_acc': {
                'status': 'Completed',
                'remarks': 'Service account already present',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'email_notification_result_existing_service_acc': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'svc_name': 'svc_testaccount',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for existing service account")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.existing_service_account_activity_update_new_service_account', key='activity_data')
    process_id = activity_data['data'].get('process_id', '')
    info = activity_data['data'].get('activity_info_fetched', {})
    user_email = info.get('user_email', '')
    user_name = info.get('user_name', '')
    env = info.get('env', '')
    description = info.get('description', '')
    organization = info.get('organization', '')
    platform = info.get('platform', '')
    business_justification = info.get('business_justification', '')
    role = info.get('role', '')
    # service_account_name_check
    svc_name = activity_data['data']['service_account_name_check'].get('svc_name', '')

    hook.info(f"Sending existing service account notification to user: {user_email}")

    try:
        # Initialize mapping hook
        mapping_hook = MappingHook()

        # Fetch template using mapping hook
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_sr_management",
            mapping_namespace_name="access_management_enable",
            mapping_key="existing_service_account_email_template_new_service_account"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for existing service account template")

        # Get the first matching element and extract its value
        template_element = mapping_elements[0]
        if not isinstance(template_element, dict):
            raise ValueError("Invalid mapping element format")

        template_content = template_element.get("value")
        if not template_content:
            raise ValueError("No template content found in mapping element")

        # Transform data to match template placeholders
        template_data = {
            "user_name": user_name,
            "process_id": process_id,
            "svc_name": svc_name,
            "env": env,
            "description": description,
            "organization": organization,
            "platform": platform,
            "business_justification": business_justification,
            "role": role
        }

        # Render content
        rendered_content = Template(template_content).render(**template_data)

        # Send email using notify hook
        subject = f"Access Management | Service Account Creation | Process Id: {process_id}"
        notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
        email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")

        try:
            notify_hook = NotifyHook()
            result = notify_hook.send_notification({
                "name": notifier_name,
                "subject": subject,
                "message": rendered_content,
                "cc_notifier_id": [email_group],
                "notifier_id": user_email
            })
            hook.info("Send email notification task initiated")

            # Update activity_data with success information
            activity_data['data']['email_notification_result_existing_service_acc'] = {
                'status': 'Sent',
                'recipients': [user_email],
                'svc_name': svc_name,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": result
            }
        except httpx.HTTPError as e:
            error_msg = f"Failed to send email notification: {str(e)}"
            hook.error(error_msg)
            # Update activity_data with error information
            activity_data['data']['email_notification_result_existing_service_acc'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            raise ValueError(error_msg)

    except Exception as e:
        error_msg = f"Error sending email notification: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['email_notification_result_existing_service_acc'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        raise ValueError(error_msg)

def update_activity_status_ad_process_error_new_service_account(**context) -> Dict[str, Any]:
    """
    Update activity status for AD process error.

    This function:
    1. Gets worklog_id from XCom
    2. Gets process_id and AD output from activity_data
    3. Updates EXT_ACCESS_MANAGEMENT_ACTIVITY table with error status and remarks
    4. Updates activity_data with the update result

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished with error',
                'AD_OUTPUT': 'Unexpected error occurred'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished with error',
                'AD_OUTPUT': 'Unexpected error occurred'
            },
            'activity_update_result_ad_process_error': {
                'status': 'Error in ITSM Process',
                'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting activity update for AD process error")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.update_activity_status_ad_process_error_new_service_account', key='activity_data')
    info = activity_data['data'].get('activity_info_fetched', {})
    ad_result = activity_data['data'].get('ad_process_result', {})
    user_email = info.get('user_email', '')
    user_name = info.get('user_name', '')
    svc_name = activity_data['data']['service_account_name_check'].get('svc_name', '')
    env = info.get('env', '')
    description = info.get('description', '')
    organization = info.get('organization', '')
    platform = info.get('platform', '')
    business_justification = info.get('business_justification', '')
    role = info.get('role', '')
    ad_output = ad_result.get('AD_OUTPUT', '')
    python_status = ad_result.get('PYTHON_STATUS', '')
    process_id = activity_data['data'].get('process_id', '')
    task_type = info.get('task_type', '')

    hook.info(f"Updating activity table for process_id: {process_id} - AD process error: {ad_output}")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to update activity table
    query = f"""
        UPDATE EXT_ACCESS_MANAGEMENT_ACTIVITY
        SET u_status = 'Error in ITSM Process',
            u_remarks = 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)'
        WHERE u_identifier = '{process_id}'
    """

    try:
        # Execute query
        result = db_hook.execute_query(query)

        if not result.get('success'):
            error_msg = result.get('error', 'Unknown error')
            hook.error(f"Failed to update activity table: {error_msg}")
            # Update activity_data with error information
            activity_data['data']['activity_update_result_ad_process_error'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        hook.info(f"Successfully updated activity table for process_id: {process_id}")

        # Update activity_data with success information
        activity_data['data']['activity_update_result_ad_process_error'] = {
            'status': 'Error in ITSM Process',
            'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)',
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)

        return {
            "success": True,
            "data": {
                "status": "Error in ITSM Process",
                "remarks": "Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)"
            }
        }

    except Exception as e:
        error_msg = f"Error updating activity table: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['activity_update_result_ad_process_error'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        return {
            "success": False,
            "error": error_msg
        }

def send_communication_ad_process_error_new_service_account(**context) -> Dict[str, Any]:
    """
    Send communication for AD process error.

    This function:
    1. Gets worklog_id from XCom
    2. Gets user data and service account details from activity_data
    3. Prepares email content using template from mapping
    4. Sends email notification using email hook

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished with error',
                'AD_OUTPUT': 'Unexpected error occurred'
            },
            'activity_update_result_ad_process_error': {
                'status': 'Error in ITSM Process',
                'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'ad_process_result': {
                'PYTHON_STATUS': 'Finished with error',
                'AD_OUTPUT': 'Unexpected error occurred'
            },
            'activity_update_result_ad_process_error': {
                'status': 'Error in ITSM Process',
                'remarks': 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'email_notification_result_ad_process_error': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'svc_name': 'svc_testaccount',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for AD process error")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.update_activity_status_ad_process_error_new_service_account', key='activity_data')
    info = activity_data['data'].get('activity_info_fetched', {})
    ad_result = activity_data['data'].get('ad_process_result', {})
    user_email = info.get('user_email', '')
    user_name = info.get('user_name', '')
    svc_name = activity_data['data']['service_account_name_check'].get('svc_name', '')    
    env = info.get('env', '')
    description = info.get('description', '')
    organization = info.get('organization', '')
    platform = info.get('platform', '')
    business_justification = info.get('business_justification', '')
    role = info.get('role', '')
    ad_output = ad_result.get('AD_OUTPUT', '')
    python_status = ad_result.get('PYTHON_STATUS', '')
    process_id = activity_data['data'].get('process_id', '')
    task_type = info.get('task_type', '')

    hook.info(f"Sending AD process error notification to user: {user_email}")

    try:
        # Initialize mapping hook
        mapping_hook = MappingHook()

        # Fetch template using mapping hook
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_sr_management",
            mapping_namespace_name="access_management_enable",
            mapping_key="ad_process_error_email_template_common"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for AD process error template")

        # Get the first matching element and extract its value
        template_element = mapping_elements[0]
        if not isinstance(template_element, dict):
            raise ValueError("Invalid mapping element format")

        template_content = template_element.get("value")
        if not template_content:
            raise ValueError("No template content found in mapping element")

        # Transform data to match template placeholders
        template_data = {
            "user_name": user_name,
            "svc_name": svc_name,
            "env": env,
            "task_type": task_type,
            "process_id": process_id,
            "description": description,
            "organization": organization,
            "platform": platform,
            "business_justification": business_justification,
            "role": role,
            "ad_output": ad_output,
            "python_status": python_status
        }

        # Render content
        rendered_content = Template(template_content).render(**template_data)

        # Send email using notify hook
        subject = f"Access Management | Error in AD Check | Process Id: {process_id}"
        notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
        email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")

        try:
            notify_hook = NotifyHook()
            result = notify_hook.send_notification({
                "name": notifier_name,
                "subject": subject,
                "message": rendered_content,
                "cc_notifier_id": [email_group],
                "notifier_id": user_email  # Send to both user and manager
            })
            hook.info("Send email notification task initiated")

            # Update activity_data with success information
            activity_data['data']['email_notification_result_ad_process_error'] = {
                'status': 'Sent',
                'recipients': [user_email],
                'svc_name': svc_name,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": result
            }
        except httpx.HTTPError as e:
            error_msg = f"Failed to send email notification: {str(e)}"
            hook.error(error_msg)
            # Update activity_data with error information
            activity_data['data']['email_notification_result_ad_process_error'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            raise ValueError(error_msg)

    except Exception as e:
        error_msg = f"Error sending email notification: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['email_notification_result_ad_process_error'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        raise ValueError(error_msg)

def sr_creation_failed_activity_update_new_service_account(**context) -> Dict[str, Any]:
    """
    Update activity data when SR creation fails.

    This function:
    1. Gets worklog_id from XCom
    2. Gets process_id and error details from activity_data
    3. Updates activity_data with failure information
    4. Updates EXT_ACCESS_MANAGEMENT_ACTIVITY table with error status

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'sr_creation_result': {
                'status': 'Failed',
                'error': 'Failed to create service request'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'sr_creation_result': {
                'status': 'Failed',
                'error': 'Failed to create service request'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_failed_activity_update': {
                'status': 'Failed',
                'error': 'Failed to create service request',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting activity update for failed SR creation")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.sr_creation_new_service_account', key='activity_data')
    process_id = activity_data['data']['process_id']
    error_msg = activity_data['data']['sr_creation_result'].get('error', 'Unknown error')

    hook.info(f"Updating activity data for failed SR creation. Process ID: {process_id}")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to update activity table
    query = f"""
        UPDATE EXT_ACCESS_MANAGEMENT_ACTIVITY
        SET u_status = 'Error in ITSM Process',
            u_remarks = 'Transfer to FO team. ITSM error in SR submission. Validation successful for request (New Service Account)'
        WHERE u_identifier = '{process_id}'
    """

    try:
        # Execute query
        result = db_hook.execute_query(query)

        if not result.get('success'):
            error_msg = result.get('error', 'Unknown error')
            hook.error(f"Failed to update activity table: {error_msg}")
            # Update activity_data with error information
            activity_data['data']['sr_creation_failed_activity_update'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            raise ValueError(f"Failed to update activity table: {error_msg}")

        hook.info(f"Successfully updated activity status for process_id: {process_id}")

        # Update activity_data with success information
        activity_data['data']['sr_creation_failed_activity_update'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)

        return {
            "success": True,
            "data": {
                "status": "Failed",
                "error": error_msg
            }
        }

    except Exception as e:
        error_msg = f"Error updating activity table: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['sr_creation_failed_activity_update'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        raise ValueError(error_msg)

def sr_creation_failed_email_notification_new_service_account(**context) -> Dict[str, Any]:
    """
    Send communication for failed service request creation.

    This function:
    1. Gets worklog_id from XCom
    2. Gets user data and service account details from activity_data
    3. Prepares email content using template from mapping
    4. Sends email notification using email hook

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'status': 'Failed',
                'error': 'Failed to create service request'
            },
            'sr_creation_failed_activity_update': {
                'status': 'Failed',
                'error': 'Failed to create service request',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name': 'svc_testaccount'
            },
            'activity_info_fetched': {
                'task_type': 'New Service Account',
                'svc_name': 'svc_testaccount',
                'user_name': 'John Doe',
                'user_email': 'john.doe@company.com',
                'manager_name': 'Jane Smith',
                'manager_email': 'jane.smith@company.com',
                'env': 'production',
                'description': 'Service account for database access',
                'organization': 'IT',
                'platform': 'Windows',
                'business_justification': 'Required for database maintenance',
                'role': 'Database Administrator'
            },
            'sr_creation_result': {
                'status': 'Failed',
                'error': 'Failed to create service request'
            },
            'sr_creation_failed_activity_update': {
                'status': 'Failed',
                'error': 'Failed to create service request',
                'timestamp': '2024-03-21T17:31:05.948'
            },
            'email_notification_result_sr_creation_failed': {
                'status': 'Sent',
                'recipients': ['john.doe@company.com'],
                'svc_name': 'svc_testaccount',
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for failed SR creation")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.sr_creation_failed_activity_update_new_service_account', key='activity_data')
    info = activity_data['data'].get('activity_info_fetched', {})
    sr_result = activity_data['data'].get('sr_creation_result', {})
    user_email = info.get('user_email', '')
    user_name = info.get('user_name', '')
    svc_name = activity_data['data']['service_account_name_check'].get('svc_name', '')    
    env = info.get('env', '')
    description = info.get('description', '')
    organization = info.get('organization', '')
    platform = info.get('platform', '')
    business_justification = info.get('business_justification', '')
    role = info.get('role', '')
    process_id = activity_data['data'].get('process_id', '')
    sr_number = sr_result.get('request_number', '')
    task_type = info.get('task_type', '')
    error_msg = sr_result.get('error', 'Unknown error')

    hook.info(f"Sending SR creation failure notification to user: {user_email}")

    try:
        # Initialize mapping hook
        mapping_hook = MappingHook()

        # Fetch template using mapping hook #sr_creation_failed_email_template_new_service_account will be common
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_sr_management",
            mapping_namespace_name="access_management_enable",
            mapping_key="sr_creation_failed_email_template_common"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for SR creation failure template")

        # Get the first matching element and extract its value
        template_element = mapping_elements[0]
        if not isinstance(template_element, dict):
            raise ValueError("Invalid mapping element format")

        template_content = template_element.get("value")
        if not template_content:
            raise ValueError("No template content found in mapping element")

        # Transform data to match template placeholders
        template_data = {
            "user_name": user_name,
            "svc_name": svc_name,
            "env": env,
            "description": description,
            "organization": organization,
            "platform": platform,
            "business_justification": business_justification,
            "role": role,
            "error_message": error_msg,
            "task_type": task_type,
            "process_id": process_id,
        }

        # Render content
        rendered_content = Template(template_content).render(**template_data)

        # Send email using notify hook
        subject = f"Service Request Creation Failed - Service Account {svc_name}"
        notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
        email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")

        try:
            notify_hook = NotifyHook()
            result = notify_hook.send_notification({
                "name": notifier_name,
                "subject": subject,
                "message": rendered_content,
                "cc_notifier_id": [email_group],
                "notifier_id": user_email
            })
            hook.info("Send email notification task initiated")

            # Update activity_data with success information
            activity_data['data']['email_notification_result_sr_creation_failed'] = {
                'status': 'Sent',
                'recipients': [user_email],
                'svc_name': svc_name,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": result
            }
        except httpx.HTTPError as e:
            error_msg = f"Failed to send email notification: {str(e)}"
            hook.error(error_msg)
            # Update activity_data with error information
            activity_data['data']['email_notification_result_sr_creation_failed'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            raise ValueError(error_msg)

    except Exception as e:
        error_msg = f"Error sending email notification: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['email_notification_result_sr_creation_failed'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        raise ValueError(error_msg)

def send_comm_service_account_name_change(**context) -> Dict[str, Any]:
    """
    Send communication for service account name creation failure.

    This function:
    1. Gets activity_data from XCom
    2. Gets essential user details from EXT_ACCESS_MANAGEMENT_ACTIVITY table
    3. Prepares email content using template from mapping
    4. Sends email notification using email hook

    Example input (from activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name_creation_failed': 'Service account name exceeds 20 characters'
            }
        }
    }

    Example output (added to activity_data):
    {
        'data': {
            'process_id': '20250321173105948',
            'worklog_id': 'WL12345',
            'service_account_name_check': {
                'svc_name_creation_failed': 'Service account name exceeds 20 characters'
            },
            'email_notification_result_name_change': {
                'status': 'Sent',
                'recipients': ['user@company.com', 'manager@company.com'],
                'timestamp': '2024-03-21T17:31:05.948'
            }
        }
    }
    """
    ti = context['ti']

    # Get worklog_id from XCom
    worklog_id = ti.xcom_pull(task_ids='create_worklog', key='worklog_id')
    if not worklog_id:
        logger.error("No worklog ID found in XCom")
        return {
            "success": False,
            "error": "No worklog ID found in XCom"
        }

    # Initialize worklog hook
    try:
        hook = WorkLogHook()
        hook.set_worklog_id(worklog_id)
        hook.info("Starting email notification for service account name creation failure")
    except Exception as e:
        logger.error(f"[Worklog: {worklog_id}] Error initializing worklog hook: {str(e)}")
        return {
            "success": False,
            "error": f"Error initializing worklog hook: {str(e)}"
        }

    # Get activity data
    activity_data = ti.xcom_pull(task_ids='new_service_account_group.check_service_account_name_new_service_account', key='activity_data')
    process_id = activity_data['data']['process_id']
    failure_message = activity_data['data']['service_account_name_check']['svc_name_creation_failed']

    hook.info(f"Fetching user details for process_id: {process_id}")

    # Get connection id from environment variable
    oscr_connection_id = os.environ.get("OSCAR_DB_EXT_CONNECTION_ID", "oscar_db_ext")

    # Get database connection
    db_hook = AccessManagementSQLHook(connection_id=oscr_connection_id, worklog_id=worklog_id)

    # Query to fetch only essential user details
    query = f"""
        SELECT
            u_user_name,
            u_user_email,
            u_manager_name,
            u_manager_email,
            u_svc_name,
            u_env
        FROM EXT_ACCESS_MANAGEMENT_ACTIVITY
        WHERE u_identifier = '{process_id}'
    """

    try:
        # Execute query
        result = db_hook.get_row_data(query)

        if not result.get('success') or not result.get('data'):
            error_msg = f"No activity found for process_id: {process_id}"
            hook.error(error_msg)
            activity_data['data']['email_notification_result_name_change'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            return {
                "success": False,
                "error": error_msg
            }

        # Extract user details from result
        user_name = result['data'][0]
        user_email = result['data'][1]
        manager_name = result['data'][2]
        manager_email = result['data'][3]
        svc_name = result['data'][4]
        env = result['data'][5]

        hook.info(f"Sending service account name creation failure notification to user: {user_email}")

        # Initialize mapping hook
        mapping_hook = MappingHook()

        # Fetch template using mapping hook
        mapping_elements = mapping_hook.list_mapping_elements(
            mapping_name="access_management_sr_management",
            mapping_namespace_name="access_management_enable",
            mapping_key="service_account_name_change_email_template_new_service_account"
        )

        if not mapping_elements:
            raise ValueError("No mapping element found for service account name change template")

        # Get the first matching element and extract its value
        template_element = mapping_elements[0]
        if not isinstance(template_element, dict):
            raise ValueError("Invalid mapping element format")

        template_content = template_element.get("value")
        if not template_content:
            raise ValueError("No template content found in mapping element")

        # Transform data to match template placeholders with only essential fields
        template_data = {
            "svc_name": svc_name,
            "process_id": process_id,
            "identifier": process_id,
            "environment": env  # Use the environment from the request instead of BUILD_MODE
        }

        # Render content
        rendered_content = Template(template_content).render(**template_data)

        # Send email using notify hook
        subject = f"Access Management | Service Account Creation | Process Id: {process_id}"
        notifier_name = os.environ.get("OSCAR_ENABLE_NOTFIER_NAME", "oscar_notifier_email")
        email_group = os.environ.get("OSCAR_ENABLE_EMAIL_GROUP", "am_email_group")

        try:
            notify_hook = NotifyHook()
            result = notify_hook.send_notification({
                "name": notifier_name,
                "subject": subject,
                "message": rendered_content,
                "cc_notifier_id": [email_group, manager_email],
                "notifier_id": user_email  # Send to both user and manager
            })
            hook.info("Send email notification task initiated")

            # Update activity_data with success information
            activity_data['data']['email_notification_result_name_change'] = {
                'status': 'Sent',
                'recipients': [user_email, manager_email],
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)

            return {
                "success": True,
                "data": result
            }
        except httpx.HTTPError as e:
            error_msg = f"Failed to send email notification: {str(e)}"
            hook.error(error_msg)
            # Update activity_data with error information
            activity_data['data']['email_notification_result_name_change'] = {
                'status': 'Failed',
                'error': error_msg,
                'timestamp': datetime.now().isoformat()
            }
            ti.xcom_push(key='activity_data', value=activity_data)
            raise ValueError(error_msg)

    except Exception as e:
        error_msg = f"Error sending email notification: {str(e)}"
        hook.error(error_msg)
        # Update activity_data with error information
        activity_data['data']['email_notification_result_name_change'] = {
            'status': 'Failed',
            'error': error_msg,
            'timestamp': datetime.now().isoformat()
        }
        ti.xcom_push(key='activity_data', value=activity_data)
        raise ValueError(error_msg)
